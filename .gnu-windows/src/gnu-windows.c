// Tiny, standalone launcher for gnu_windows
// * Sets $gnu_windows to the release version (-DVERSION)
// * Sets $gnu_windows_HOME to the install location
// * Maybe sets $HOME according to gnu_windows.ini
// * Starts a login shell with "sh -l"
//
// $ gcc -DVERSION="$VERSION" -nostartfiles -fno-builtin
//       -o gnu_windows.exe gnu_windows.c
//
...
{
    MessageBoxW(0, msg, u"gnu_windows launcher", 0x10);
    ExitProcess(2);
}
...
// Read and process "gnu_windows.home" from "gnu_windows.ini". Environment
...
static c16 *homeconfig(arena *perm, arena scratch)
{
    ...
    for (symbol section = 0, key = 0;;) {
        ...
        case INI_key:
            ...
            continue;
        case INI_value:
            if (!home && section==sym_gnu_windows && key==sym_home) {
...
static u32 gnu_windows(void)
{
    ...
    SetEnvironmentVariableW(u"gnu_windows_HOME", path.buf);  // ignore errors

    // Maybe set HOME from gnu_windows.ini
    ...
    #define XSTR(s) u ## # s
    SetEnvironmentVariableW(u"gnu_windows", LSTR(VERSION));  // ignore errors
    #endif

    // Set the console title as late as possible, but not after starting
    // the shell because .profile might change it.
    SetConsoleTitleW(u"gnu_windows");  // ignore errors
